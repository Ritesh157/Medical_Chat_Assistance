...
from app.vectorstore_utils import create_faiss_index, retrieve_relavant_docs
...
"""
Now set page configuration (page layout) using Streamlit
"""
    ...
    if st.button("🚀 Process Documents", type="primary"):
       with st.spinner("Processing your medical documents..."):
        ...
        for text in all_texts:
            chunk.extend(text_splitter.split_text(text))
            
        # Create FAISS index
        vectorstore = create_faiss_index(chunk)
    ...
    if st.session_state.vectorstore and st.session_state.chat_model:
        with st.chat_message("assistant"):
            with st.spinner("🔍 Searching documents..."):
                ...
                relevant_docs = retrieve_relavant_docs(st.session_state.vectorstore, prompt)

                # Create context from relevant docs
                content = "\n\n".join([doc.page_content for doc in relevant_docs])

                # Create prompt from context
                system_prompt = f"""You are MediChat Pro, an intelligent medical document assistan.
